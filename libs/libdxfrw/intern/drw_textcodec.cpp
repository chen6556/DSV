...
std::string DRW_Converter::toUtf8(const std::string &s)
{
    ...
    for (size_t i = 0, count = s.length(); i < count; ++i)
    {
        ...
        if (c < 0x80)
        { // ascii check for /U+????
            if (c == '\\' && i + 6 < count && s.at(i + 1) == 'U' && s.at(i + 2) == '+')
            {
                result.append(s.substr(j, i - j));
                result.append(encodeText(s.substr(i, 7)));
                i += 6;
                j = i + 1;
            }
        }
    ...
    for (size_t i = 0, count = s.length(); i < count; ++i)
    {
        ...
        { // need to decode
            ...
            for (int k = 0; k < cpLenght; k++)
            {
                if (table[k] == code)
                {
                    result.push_back(CPOFFSET + k); // translate from table
                    notFound = false;
                    break;
                }
            }
    ...
    for (std::string::const_iterator it = s.begin(), end = s.end(); it != end; ++it)
    {
        ...
        if (c < 0x80)
        {
            ...
            if (c == '\\')
            {
                ...
                {
                    res.append(encodeText(std::string(it, it + 7)));
                    it += 6;
                }
                else
                {
                    res.push_back(c); // no \U+ encoded text write
                }
            }
            else
            {
                res.push_back(c); // c!='\' ascii char write
            }
        }
        else
        {                                      // end c < 0x80
            res.append(encodeNum(table[c - 0x80])); // translate from table
        }
    } // end for
...
std::string DRW_ConvDBCSTable::toUtf8(const std::string &s)
{
    ...
    for (std::string::const_iterator it = s.begin(), end = s.end(); it != end; ++it)
    {
        ...
        { // 1 byte table
            notFound = false;
            res.append(encodeNum(0x20AC)); // euro sign
        }
        else
        { // 2 bytes
            ++it;
            ...
            for (int k = sta; k < end; k++)
            {
                ...
                {
                    res.append(encodeNum(doubleTable[k][1])); // translate from table
                    notFound = false;
                    break;
                }
            }
        ...
        if (notFound)
        {
            res.append(encodeNum(NOTFOUND936));
        }
    } // end for
...
std::string DRW_Conv932Table::fromUtf8(const std::string &s)
{
    ...
    for (size_t i = 0, count = s.length(); i < count; ++i)
    {
        ...
        { // need to decode
            ...
            {
                result.push_back(code - CPOFFSET932); // translate from table
                notFound = false;
            }
                             ...
                             (code > 0x200F && code < 0x9FA1) || code > 0xF928))
            {
                for (int k = 0; k < cpLenght; k++)
                {
                    if (doubleTable[k][1] == code)
                    {
                        ...
                        d[1] = data & 0xFF;
                        d[2] = '\0';
                        result.append(d); // translate from table
                        notFound = false;
                        break;
                    }
                }
...
std::string DRW_Conv932Table::toUtf8(const std::string &s)
{
    ...
    for (std::string::const_iterator it = s.begin(), end = s.end(); it != end; ++it)
    {
        ...
        { // 1 byte table
            notFound = false;
            res.append(encodeNum(c + CPOFFSET932)); // translate from table
        }
        else
        { // 2 bytes
            ++it;
            ...
            if (end > 0)
            {
                for (int k = sta; k < end; k++)
                {
                    ...
                    {
                        res.append(encodeNum(DRW_DoubleTable932[k][1])); // translate from table
                        notFound = false;
                        break;
                    }
                }
        ...
        if (notFound)
        {
            res.append(encodeNum(NOTFOUND932));
        }
    } // end for
