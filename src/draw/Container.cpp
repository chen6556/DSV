    ...
    : _text(text), _text_size(size)
{
    ...
    {
        width = std::max<long long>(width, font.pixelSize() * s.length());
    }
    if (width == 0)
    {
        width = font.pixelSize() * text.length();
    }
    width = std::max<long long>(20ll, width);
    long long height = std::max<long long>(font_metrics.lineSpacing() * (text.count('\n') + 1), 20ll);
...
void Text::set_text(const QString &str, const int size)
{
    ...
    width = std::max<long long>(20ll, width);
    long long height = std::max<long long>(font_metrics.lineSpacing() * (str.count('\n') + 1), 20ll);
...
void Text::update_size(const int size)
{
    ...
    width = std::max<long long>(20ll, width);
    long long height = std::max<long long>(font_metrics.lineSpacing() * (_text.count('\n') + 1), 20ll);
    set_left(coord.x - 1 - width / 2);
    set_right(coord.x + 1 + width / 2);
    set_top(coord.y + height / 2);
    set_bottom(coord.y- height / 2);
