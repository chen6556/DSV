                        ...
                        const bool infinite, std::vector<std::tuple<size_t, double, double, double>> *tvalues)
{
    ...
    for (size_t i = 0, end = bezier.control_points.size() - order; i < end; i += order)
    {
        ...
        for (Geo::Point &point : temp)
        {
            ...
            std::function<double(const double)> f = [&](const double t)
            {
                Geo::Point coord;
                for (int j = 0; j <= order; ++j)
                {
            ...
            const double tdis = f(t);
            min_dis = f(lower);
            while (lower > 0.0 && min_dis < tdis)
            {
                lower -= 1e-4;
                min_dis = f(lower);
            }
...
bool Geo::is_intersected(const AABBRectParams &params0, const AABBRectParams &params1, const bool inside)
{
    ...
    {
        return !(
            (params0.top < params1.top && params0.right < params1.right && params0.bottom > params1.bottom &&
             params0.left > params1.left) ||
            (params1.top < params0.top && params1.right < params0.right && params1.bottom > params0.bottom && params1.left > params0.left));
    }
}
...
int Geo::is_intersected(const Circle &circle0, const Circle &circle1, Point &point0, Point &point1)
{
    ...
    if (distance <= circle0.radius + circle1.radius && distance >= std::abs(circle0.radius - circle1.radius))
    {
        ...
        if (distance == circle0.radius + circle1.radius || distance == std::abs(circle0.radius - circle1.radius))
        {
            if (circle0.x == circle1.x && circle0.y == circle1.y)
            {
                return 0;
            }
            point0.x = x;
                        ...
                        std::vector<std::tuple<double, double, double>> *tvalues)
{
    ...
    for (size_t n = 0, count = values.size(); n < count; ++n)
    {
        ...
        const std::function<double(const double)> f = [&](const double t)
        {
            ...
            return Geo::distance(coord, ellipse) * 1e9;
        };
        const double tdis = f(t);
        min_dis = f(lower);
        while (lower > 0.0 && min_dis < tdis)
        ...
        for (size_t q = 0, end1 = bezier1.control_points.size() - order; q < end1; q += order)
        {
            ...
            for (size_t a = 1, count0 = polyline0.size(); a < count0; ++a)
            {
                for (size_t b = 1, count1 = polyline1.size(); b < count1; ++b)
                {
                    ...
                    if (tvalues0 != nullptr)
                    {
                        tvalues0->emplace_back(p / order, t0, point0.x, point0.y);
                    }
                    if (tvalues1 != nullptr)
                    {
                        tvalues1->emplace_back(q / order, t1, point1.x, point1.y);
                    }
                }
            }
        }
                        ...
                        std::vector<std::tuple<double, double, double>> *tvalues1)
{
    ...
    for (size_t i = 0, count = bezier_values.size(); i < count; ++i)
    {
        ...
        if (Geo::Point point((point0 + point1) / 2); std::find(intersections.begin(), intersections.end(), point) == intersections.end())
        {
            ...
            if (tvalues0 != nullptr)
            {
                tvalues0->emplace_back(std::get<0>(bezier_values[i]) / order, t0, point0.x, point0.y);
            }
            if (tvalues1 != nullptr)
            {
                tvalues1->emplace_back(t1, point1.x, point1.y);
