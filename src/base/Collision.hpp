...
bool gjk(const Geo::Point &point01, const Geo::Point &point02, const Geo::Point &point03, const Geo::Point &point11,
...
template <typename T = GridMap> class CollisionDetector
{
private:
    T _detector;

    static bool pair_in_pairs(const std::vector<std::pair<Geo::Polygon *, Geo::Polygon *>> &pairs, const Geo::Polygon *object0,
                              const Geo::Polygon *object1, const bool ordered = false)
    {
        if (ordered)
        {
        ...
        else
        {
            for (const std::pair<Geo::Polygon *, Geo::Polygon *> &pair : pairs)
            {
                if ((pair.first == object0 && pair.second == object1) || (pair.first == object1 && pair.second == object0))
                {
                    return true;
                }
            }
...
public:
    ...
    CollisionDetector(const std::vector<Geo::Polygon *> &objects) : _detector(objects)
    {
    }

    CollisionDetector(const std::initializer_list<Geo::Polygon *> &objects) : _detector(objects)
    {
    }
