...
static Parser<std::string> end = +(ch_p(';') | ch_p('\n') | eol_p());
...
static Parser<bool> pu = str_p("PU")[pu_a] >> !list_p(coord, separator) >> end;
static Parser<bool> pd = str_p("PD")[pd_a] >> !list_p(coord, separator) >> end;
...
static Parser<std::string> text_end = ch_p('\x3') | ch_p('\x4') | end;
